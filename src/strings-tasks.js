...
function removeFirstOccurrences(str, value) {
  ...
  return str.includes(value)
    ? str.slice(0, indexStart) + str.slice(indexEnd)
    : str;
}
...
function isPalindrome(str) {
  const charsToReplace = /[\s,.!?;:]/g;
  const strLowerCase = str.toLowerCase().replaceAll(charsToReplace, '');
  return strLowerCase.split('').reverse().join('') === strLowerCase;
}
...
function findLongestWord(sent) {
  const wordsArr = sent.split(' ').sort((a, b) => b.length - a.length);
  return wordsArr[0];
}
...
function extractNameFromTemplate(value) {
  return value ? value.substring(7, value.length - 1) : '';
}
...
function encodeToRot13(str) {
  ...
  const encodeChars = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let newString = '';
  for (let i = 0; i < str.length; i += 1) {
    const charIndex = chars.indexOf(str[i]);
    newString += charIndex !== -1 ? encodeChars[charIndex] : str[i];
  }
  return newString;
}
...
function getCardId(str) {
  const suit = '♣♦♥♠';
  const currentSuit = str[str.length - 1];
  const suitCodeNum = suit.indexOf(currentSuit);
  ...
  const cardNum = +str
    ...
    .replace('Q', 12)
    .replace('K', 13);
  const cardNumToZero = cardNum - 1;
  return cardsInLine * suitCodeNum + cardNumToZero;
}
